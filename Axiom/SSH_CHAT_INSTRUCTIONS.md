...
```bash
ssh -p 22222 -i ~/.ssh/bitrix_root_ed25519 \
  -o BatchMode=yes \
  -o StrictHostKeyChecking=accept-new \
  root@185.51.60.114 "whoami && hostname && pwd"
```

Ожидаемо: `root`, `<hostname_сервера>`, `<рабочий_каталог_сессии>`.
...
rsync -az --delete \
  --exclude 'storage/**' \
  -e "ssh -p 22222 -i $HOME/.ssh/bitrix_root_ed25519 -o BatchMode=yes -o StrictHostKeyChecking=accept-new" \
  "local/akademia-profi/academyprofi-catalog-app/" \
  root@185.51.60.114:/home/bitrix/www/local/akademia-profi/academyprofi-catalog-app/
```

Права/владелец (если нужно):

```bash
ssh -p 22222 -i ~/.ssh/bitrix_root_ed25519 root@185.51.60.114 -o BatchMode=yes \
...
```bash
ssh-keygen -R "[185.51.60.114]:22222"
```
