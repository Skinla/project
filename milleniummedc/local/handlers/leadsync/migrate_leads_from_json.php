 ...
 * Run on box via SSH or from webhook. Input: JSON array of {cloud_lead_id?, box_lead_id?, update_only?, lead, activities, comments, products}
...
if (isset($input['items']) && is_array($input['items'])) {
    $items = $input['items'];
    $rawCheck = $input['check_date_modify'] ?? true;
    if (is_bool($rawCheck)) {
        $checkDateModify = $rawCheck;
    } elseif (is_int($rawCheck)) {
        $checkDateModify = $rawCheck !== 0;
    } else {
        $s = strtolower(trim((string)$rawCheck));
        $checkDateModify = !in_array($s, ['0', 'false', 'n', 'no', 'off'], true);
    }
