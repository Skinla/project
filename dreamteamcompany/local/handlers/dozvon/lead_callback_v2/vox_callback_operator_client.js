...
function startOperatorLeg(payload) {
    ...
    if (payload.operator_destination_type === 'sip' && payload.operator_destination) {
        ...
        return VoxEngine.callSIP(payload.operator_destination, buildSipCallOptions(payload));
    }

    if (payload.operator_destination_type === 'user' && payload.operator_destination) {
        Logger.write('callback operator leg via user destination=' + payload.operator_destination);
        return VoxEngine.callUser(String(payload.operator_destination), payload.sip_line);
    }

    return VoxEngine.callUser(String(payload.operator_user_id), payload.sip_line);
}

function buildSipCallOptions(payload) {
    var host = extractSipHost(payload.operator_destination || '');
    var callerId = payload.sip_line || '';

    if (host && callerId && callerId.indexOf('@') === -1 && callerId.indexOf('sip:') !== 0) {
        callerId = 'sip:' + callerId + '@' + host;
    }

    return {
        authUser: payload.sip_line || '',
        password: payload.sip_password || '',
        callerid: callerId,
        displayName: payload.operator_extension || ''
    };
}

function extractSipHost(destination) {
    var raw = String(destination || '').replace(/^sip:/i, '');
    var parts = raw.split('@');
    if (parts.length < 2) {
        return '';
    }
