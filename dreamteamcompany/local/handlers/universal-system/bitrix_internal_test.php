...
//   /local/handlers/universal/universal-system/bitrix_internal_test.php?token=XXX
//
// РЕКОМЕНДУЕТСЯ:
...
function getLeadFieldsForTest($leadId): array
{
    ...
    return [
        ...
        'UF_CRM_5FC49F7DA5470' => $leadData['UF_CRM_5FC49F7DA5470'] ?? '', // Инфоповод
...
function testCrmLeadClassic(): void
{
    ...
    // Инфоповод (UF_CRM_5FC49F7DA5470)
    if (!empty($properties['PROPERTY_194']['VALUE'])) {
        $fields['UF_CRM_5FC49F7DA5470'] = (string)$properties['PROPERTY_194']['VALUE'];
    }
    ...
    t_print_result('CCrmLead::Add (создание тестового лида)', true, [
        ...
        'UF_CRM_5FC49F7DA5470 (Инфоповод)' => $createdLeadFields['UF_CRM_5FC49F7DA5470'] ?? 'не задано',
