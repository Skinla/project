...
class GenericNormalizer extends BaseNormalizer {
    ...
    private function buildGenericComment($data, $rawBody = '', $rawHeaders = []) {
        // Сначала проверяем наличие поля комментария в разных вариантах (используем метод из базового класса)
        $directComment = $this->extractComment($data);
        
        // Если нашли прямой комментарий, возвращаем его сразу (приоритет комментарию)
        if (!empty($directComment)) {
        ...
        // Исключаем служебные поля (убрали 'comments' чтобы обрабатывать его как комментарий)
        $excludeFields = [
            ...
            'result', 'paymentAmount', 'delivery', 'Checkbox', 'Checkbox_2', 'ASSIGNED_BY_ID'
        ];
        
        // Собираем только обработанные данные (вопросы-ответы)
        $lines = [];
        
        // Если есть прямой комментарий, добавляем его первым
        if (!empty($directComment)) {
            $lines[] = $directComment;
        }
        
        // Список полей комментариев для исключения
        $commentFields = ['COMMENTS', 'Comment', 'comment', 'comments', 'COMMENT', 'Commentary', 'commentary', 'COMMENTARY', 'message', 'Message', 'MESSAGE', 'note', 'Note', 'NOTE', 'description', 'Description', 'DESCRIPTION'];
        
        foreach ($data as $key => $value) {
            // Пропускаем служебные поля и уже обработанные поля комментариев
            if (in_array($key, $excludeFields) || in_array(strtolower($key), array_map('strtolower', $commentFields))) {
                continue;
            }
