...
$ib54 = $isCallTouch
    ? v3_getIb54ByCallTouch($inputData['subPoolName'], $calltouchSiteId)
...
if (!$ib54) {
    ...
    $chatMsg = "🚨 Домен не найден в ИБ54\n"
        . "📁 Домен: $domain\n📱 Телефон: $phone\n"
        . "⏰ " . date('d.m.Y H:i:s');
    v3_sendChat($chatMsg, $v3Config);
...
if (!empty($props['PROPERTY_195']['VALUE'])) {
    ...
    $observerIds = array_values(array_filter(array_map('intval', $observerIds), fn($v) => $v > 0));
...
// =========================================================================
// 8. crm.lead.add через REST API
// =========================================================================
$apiResult = v3_createLead($leadFields, $v3Config);
$elapsed = round(microtime(true) - $v3StartTime, 2);

if ($apiResult && isset($apiResult['result'])) {
    $leadId = $apiResult['result'];
    v3_log('OK', "lead=$leadId phone={$normalized['phone']} domain=$domain {$elapsed}s", $v3Config);
    ...
    v3_jsonResponse(['status' => 'ok', 'lead_id' => $leadId]);
}

// API вернул ошибку -- сохраняем для retry
v3_log('RETRY', "api_error phone={$normalized['phone']} domain=$domain {$elapsed}s", $v3Config);
...
// --------------- crm.lead.add через REST API ---------------

function v3_createLead(array $fields, array $config): ?array {
...
function v3_sendChat(string $message, array $config): void {
    ...
    $postData = http_build_query([
        'DIALOG_ID' => $config['error_chat_id'],
        'MESSAGE' => $message,
    ]);
    $ch = curl_init();
    curl_setopt_array($ch, [
