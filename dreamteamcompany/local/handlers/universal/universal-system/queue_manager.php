...
function processAllQueues($config) {
    ...
    error_log("queue_manager: начинаем обработку всех очередей");
    
    try {
        // Этап 1: Определение типа данных
        processRawFiles($config);
    } catch (Throwable $e) {
        logMessage("queue_manager: ОШИБКА в processRawFiles: " . $e->getMessage() . " | File: " . $e->getFile() . " | Line: " . $e->getLine(), $config['global_log'], $config);
        // Продолжаем обработку других очередей
    }
    ...
    try {
        ...
        logMessage("queue_manager: ОШИБКА в processDetectedFiles: " . $e->getMessage() . " | File: " . $e->getFile() . " | Line: " . $e->getLine(), $config['global_log'], $config);
        // Продолжаем обработку других очередей
    }
    ...
    try {
        ...
        logMessage("queue_manager: ОШИБКА в processNormalizedFiles: " . $e->getMessage() . " | File: " . $e->getFile() . " | Line: " . $e->getLine(), $config['global_log'], $config);
