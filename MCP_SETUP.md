...
В корне репозитория также лежит **`.cursor/skills/`** — Markdown-скиллы для агента (Bitrix24, вебхуки и др.). После `git clone` эта папка уже в проекте; Cursor подхватывает skills из каталога `.cursor` в workspace. При необходимости скопируй отдельные `SKILL.md` в глобальный каталог skills, если используешь общие правила для всех проектов.
